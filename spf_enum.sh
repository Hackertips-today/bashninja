     ...
     .......... [ spfEnum ] .............
             __ ______                       
            / _|  ____|                      
  ___ _ __ | |_| |__   _ __  _   _ _ __ ___  
 / __| '_ \|  _|  __| | '_ \| | | | '_ ` _ \ 
 \__ \ |_) | | | |____| | | | |_| | | | | | |
 |___/ .__/|_| |______|_| |_|\__,_|_| |_| |_|
     | |                                     
     |_|                                     

Distro: https://hackertips.today
Git Repo: https://github.com/Hackertips-today
Date: 10/24/2024
Prereqs: spfquery

Note: If you dont have spfquery installed, its not a big deal, the git is https://github.com/trailofbits/spf-query
Clone that down and run the cmd to compile it.   Once you have dig and spfquery this script will work:

Description:
./spfEnum "mailserver.com" "sourceIP[OPT]" "sourceEmailAddy[OPT]"
The only mandatory option is the target mailserver/domain.  Keep in mind, if the domain doesnt have mail MX records
it will not work.  Try it on "gmail.com"  "mail.com" etc..
Also, you can add -all  to extensively test

All logging goes to stdout and /tmp/mailchk<timestamp>.log

Example of what you will likely see some of:
--------------
spfquery --ip="255.255.255.255" --mfrom="user@example.com" --helo="mail.com"  # cmd run by script

softfail
mail.com ... _spf.mail.com: Sender is not authorized by default to use 'mail.com' in 'helo' identity, however domain is not currently prepared for false failures 
(mechanism '~all' matched)
mail.com ... _spf.mail.com: Sender is not authorized by default to use 'mail.com' in 'helo' identity, however domain is not currently prepared for false failures 
(mechanism '~all' matched)
Received-SPF: softfail (mail.com ... _spf.mail.com: Sender is not authorized by default to use 'mail.com' in 'helo' identity, however domain is not currently prepared 
for false failures (mechanism '~all' matched)) receiver=localhost; identity=helo; helo=mail.com; client-ip=255.255.255.255
--------
